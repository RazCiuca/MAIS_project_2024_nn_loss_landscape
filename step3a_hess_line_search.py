"""
does line search in the direction of given eigenvectors for each minibatch and saves all results
...
# line search around negative eigen
if __name__ == "__main__":
    ...
    for iters in [2000]:
        ...
        eigindices = t.nonzero(eigvals < -1e-4).flatten().numpy()
        ...
        f_values = line_search_around_eigenvalues(model, data_x, data_y, loss_fn, eigvals, eigvecs, gradient,
                                                  eigindices=eigindices, n_explore=40)
        t.save(f_values, f'./models/resnet9_cifar10/line_searches/{iters}_neg_eigen.pth')
